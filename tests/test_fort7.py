...
def example_data(tmp_path):
    ...
    file_path = tmp_path / "fort.7"
    np.savetxt(file_path, np.column_stack([wavelength, flux]))

    return {"file_path": file_path, "wavelength": wavelength, "flux": flux}
...
def test_fort7_read(example_data):
    ...
    # Read the file
    fort7 = Fort7.read(example_data["file_path"])
